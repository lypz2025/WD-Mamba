...
# Version-1:
        ...
        # embedding_dim = 1536  # Set to the appropriate feature size
        # num_embeddings = 1000  # Placeholder, update according to your model's requirements
        # dim = 1536
        # drop_path_prob = 0.0   

        # # Initial Blocks - 6 x BasicMambaBlock
        # self.transformer_blocks.extend([
        #     BasicMambaBlock(
        #         norm1=FusedAddRMSNorm(norm=AdaRMSNorm(embedding_dim=embedding_dim, num_embeddings=num_embeddings), drop_path=drop_path_prob, dim=dim),  # Correct pass of drop_path_prob
        #         attn1=Mamba(
        #             in_proj=nn.Linear(1536, 6448, bias=False),
        #             conv1d=nn.Conv1d(3328, 3328, kernel_size=4, stride=1, padding=3, groups=3328),
        #             act=nn.SiLU(),
        #             norm=AdaRMSNorm(embedding_dim=embedding_dim, num_embeddings=num_embeddings),
        #             out_proj=nn.Linear(3072, 1536, bias=False)
        #         )
        #     ) for _ in range(6)  # Initial 6 blocks
        # ])

        # # Middle Block - 1 x BasicMambaBlock
        ...
        # # Output Blocks - 6 x BasicMambaBlock
        # self.transformer_blocks.extend([
        #     BasicMambaBlock(
        #         norm1=FusedAddRMSNorm(norm=AdaRMSNorm(embedding_dim=embedding_dim, num_embeddings=num_embeddings), drop_path=drop_path_prob, dim=dim),  # Correct pass of drop_path_prob
        #         attn1=Mamba(
        ...
        #             out_proj=nn.Linear(3072, 1536, bias=False)
        #         )
        #     ) for _ in range(6)  # Output 6 blocks
        # ])

        # # Output normalization layer
        # self.norm_out = FusedAddRMSNorm(norm=AdaRMSNorm(embedding_dim=embedding_dim, num_embeddings=num_embeddings), drop_path=drop_path_prob, dim=dim)  # Correct pass of drop_path_prob
