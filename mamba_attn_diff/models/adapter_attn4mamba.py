...
def generate_spiral_indices(height, width, outward=True, clockwise=True):
    if height <= 0 or width <= 0:
        return []

    grid = [[i * width + j for j in range(width)] for i in range(height)]
    result = []
    ...
    while grid:
        ...
        # Right column
        if grid and grid[0]:
            result += [row.pop(-1) for row in grid if row]
        ...
        # Left column
        if grid and grid[0]:
            result += [row.pop(0) for row in grid[::-1] if row]

    return result


def apply_spiral_pattern(x, height, width, pattern=0):
    indices = generate_spiral_indices(height, width)
    
    # Debugging log for indices
    print(f"indices: {indices}")
    
    # Handle both flat and paired indices
    if isinstance(indices[0], int):  # Flat indices
        flat_indices = indices
    else:  # Coordinate pairs
        flat_indices = [i * width + j for i, j in indices]
    
    # Apply the flat indices
    x = x[:, flat_indices, :]
    return x
...
def relative_scan_pattern_transformation(x, last_pattern, cur_pattern, height=None, dim=-2):
    '''
        ...
        def spiral-in clockwise(x, height=None): # -1
            ...
            return x
    '''
    # print("Input shape of x in relative_scan_pattern_transformation:", x.shape)

    if x.shape[dim] == 0:
        return x

    if last_pattern < 0 and cur_pattern >= 0 :
        x = x.flip(dim)
    
    if (last_pattern + cur_pattern + 4) % 2 == 1 : 
        if (cur_pattern + 4) % 2 == 1 : 
            if dim == 0:
                x = rearrange(x, '(h w) -> (w h)', h=height)
            else:
                x = rearrange(x, 'b (h w) c -> b (w h) c', h=height)
        elif (cur_pattern + 4) % 2 == 0 : 
            if dim == 0:
                x = rearrange(x, '(w h) -> (h w)', h=height)
            else:
                x = rearrange(x, 'b (w h) c -> b (h w) c', h=height)
            
    if cur_pattern < 0 and last_pattern >= 0 : 
        x = x.flip(dim)
    
    # print("Output shape of x in relative_scan_pattern_transformation:", x.shape)
    return x
...
# def relative_scan_pattern_transformation(x, last_pattern, cur_pattern, height=None, dim=-2):
#     """
#     Perform a relative transformation from the last scan pattern to the current scan pattern.

#     Args:
#         x: Input tensor of shape [batch_size, height * width, channels].
#         last_pattern: Integer indicating the last scan pattern.
#         cur_pattern: Integer indicating the current scan pattern.
#         height: Height of the image grid.
#         dim: Dimension along which to apply transformations.

#     Returns:
#         Transformed tensor according to the current scan pattern.
#     """
#     print("Input shape of x in relative_scan_pattern_transformation:", x.shape)

#     if x.shape[dim] == 0:
#         # Implement Spiral-out clockwise
#         x = apply_spiral_pattern(x, height, pattern=0)

#     if last_pattern < 0 and cur_pattern >= 0:
#         # Implement Spiral-in counterclockwise
#         x = apply_spiral_pattern(x, height, pattern=1)

#     if (last_pattern + cur_pattern + 4) % 2 == 1:
#         # Implement Spiral-out counterclockwise
#         x = apply_spiral_pattern(x, height, pattern=2)

#     if cur_pattern < 0 and last_pattern >= 0:
#         # Implement Spiral-in clockwise
#         x = apply_spiral_pattern(x, height, pattern=3)
...
class AdapterAttnForMamba(nn.Module):
    ...
    def forward_fast(self, x, pre_norm_x=None, block_idx=0, residual=None, pre_skip=None, rel_blk_id=0):
        ...
        # print(f"block_idx: {block_idx}, scan_pattern_chain length: {len(self.scan_pattern_chain)}") # updated code
...
def apply_conv2d_bld(x, conv_layer, height):
    # print("Shape of x in apply_conv2d_bld:", x.shape)
