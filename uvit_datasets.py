...
class UnlabeledDataset(Dataset):
    ...
    def __getitem__(self, item):
        try:
            ...
            if isinstance(data, tuple):  # Ignore the label if it exists
                data = data[0]  # Use only the image part
            return data
        except Exception as e:
            print(f"Error in UnlabeledDataset at index {item}: {e}")
            return None
...
#     def __init__(self, dataset, p_uncond, empty_token, resolution=None):
#         self.dataset = dataset
...
#             if isinstance(y, (int, np.int64, type(None))):  # Handle potential label
#                 if random.random() < self.p_uncond:
#                     y = self.empty_token
#             else:
#                 y = self.empty_token  # Default to empty if no valid label
#             return x, y
#         except Exception as e:
#             print(f"Error in CFGDataset at index {item}: {e}")
#             return None, None
...
class CIFAR10(DatasetFactory):
    ...
    def __init__(self, path, random_flip=False, cfg=False, p_uncond=None, is_crop=False, resolution=32):
        ...
        self.train = torchvision.datasets.CIFAR10(path, train=True, transform=transform_train, download=False)
        self.test = torchvision.datasets.CIFAR10(path, train=False, transform=transform_test, download=False)
    ...
    def fid_stat(self):
        ...
        return f'/scratch/laks/DiM-DiffusionMamba/fid/cifar10_ref_stats.npz'
...
class ImageNet256Features(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder
    ...
    def fid_stat(self):
        ...
        # return 'assets/fid_stats/fid_stats_celeba64_train_50000_ddim.npz' # updated code
...
class CelebAHQ(DatasetFactory):
    def __init__(self, path, resolution, random_crop=False, random_flip=True, cfg=False, p_uncond=None):
        ...
        print(f'Counting files from {path}')
        train_files = _list_image_files_recursively(os.path.join(path, 'train'))
        class_names = [os.path.basename(path).split("_")[0] for path in train_files]
        sorted_classes = {x: i for i, x in enumerate(sorted(set(class_names)))}
        train_labels = [sorted_classes[x] for x in class_names]
        print('Finish counting files')
    ...
    @property
    def fid_stat(self):
        # return f'assets/fid_stats/fid_stats_celeba64_train_50000_ddim.npz' # Updated code
        # return f'assets/celeba256_fid_stat.npz'
        # return f'/scratch/laks/DiM-DiffusionMamba/fid/celebahq_256_ref_stats.npz'
        return f'/scratch/laks/DiM-DiffusionMamba/fid/celebahq_256_ref_stats_new.npz'
...
class FFHQ(DatasetFactory):
    ...
    def fid_stat(self):
        # return 'assets/fid_stats/fid_stats_celeba64_train_50000_ddim.npz' # Updated code
        return f'/scratch/laks/DiM-DiffusionMamba/fid/ffhq_256_ref_stats.npz'
...
class LSUN_B(DatasetFactory):
    def __init__(self, path, resolution, random_crop=False, random_flip=True, cfg=False, p_uncond=None):
        ...
        self.train = ImageDataset(resolution, train_files, labels=train_labels, random_crop=random_crop, random_flip=random_flip)
        self.resolution = (512 if resolution[0] > 512 else resolution[0]) if isinstance(resolution, list) or isinstance(resolution, tuple) else resolution
        print("Total training samples: ",len(self.train))
        # # assert False, "{}, {}".format(self.resolution, resolution[0])
        # if len(self.train) != 1_281_167:
        #     print(f'Missing train samples: {len(self.train)} < 1281167')
    ...
    def fid_stat(self):
        # return 'assets/fid_stats/fid_stats_celeba64_train_50000_ddim.npz' # Updated code
        return '/scratch/laks/DiM-DiffusionMamba/fid/lsun_beds_ref_stats.npz'
...
class LSUN_C(DatasetFactory):
    ...
    def fid_stat(self):
        # return 'assets/fid_stats/fid_stats_celeba64_train_50000_ddim.npz' # Updated code
        return f'/scratch/laks/DiM-DiffusionMamba/fid/lsun_churches_ref_stats.npz'
...
# Updated code lines
class FFHQ_v1(DatasetFactory):
    def __init__(self, path, resolution, random_crop=False, random_flip=True, cfg=False, p_uncond=None, empty_token=None):
        super().__init__()

        self.random_crop = random_crop
        self.random_flip = random_flip

        print(f'Counting FFHQ files from {path}')
        image_files = _list_image_files_recursively(os.path.join(path, 'train'))
        print(f'Finish counting FFHQ files')

        # For FFHQ, there are no labels
        self.train = ImageDataset(resolution, image_files, labels=[], random_crop=random_crop, random_flip=random_flip)
        self.resolution = (512 if resolution[0] > 512 else resolution[0]) if isinstance(resolution, list) or isinstance(resolution, tuple) else resolution

        if len(self.train) != len(image_files):
            print(f'Missing train samples: {len(self.train)} < {len(image_files)}')

        print(f'FFHQ dataset with {len(self.train)} images')

        if cfg:  # If using classifier-free guidance
            assert p_uncond is not None
            print(f'Preparing the dataset for classifier-free guidance with p_uncond={p_uncond}')
            # Ensure empty_token is passed correctly to CFGDataset
            self.train = CFGDataset(self.train, p_uncond, empty_token, resolution=resolution)
    ...
    @property
    def fid_stat(self):
        return f'assets/fid_stats/fid_stats_celeba64_train_50000_ddim.npz'

    def sample_label(self, n_samples, device, **kwargs):
        # FFHQ dataset is unconditional, so no label sampling is necessary
        return torch.zeros(n_samples, dtype=torch.long, device=device)  # Returning dummy labels (no class labels)

    def label_prob(self, k):
        # Since FFHQ has no labels, return a dummy probability (no class probabilities)
        return 1.0
...
# Updated code lines
class ImageDataset(Dataset):
    def __init__(self, resolution, image_paths, labels=None, random_crop=False, random_flip=True):
        super().__init__()
        self.resolution = resolution
        self.image_paths = image_paths
        self.labels = labels if labels is not None else []  # Default to empty list
        self.random_crop = random_crop
        self.random_flip = random_flip
    ...
    def __getitem__(self, idx):
        try:
            resolution_idx = None
            if isinstance(idx, (tuple, list)):  # Handle multi-resolution index
                idx, resolution_idx = idx
            ...
            resolution = (
                self.resolution[resolution_idx]
                if isinstance(self.resolution, (list, tuple))
                else self.resolution
            )
            arr = process_img_input(
                pil_image, resolution, random_crop=self.random_crop, random_flip=self.random_flip
            )

            # Handle labels
            label = (
                np.array(self.labels[idx], dtype=np.int64) if self.labels else -1
            )  # -1 for unconditional

            return arr, label
        except Exception as e:
            print(f"Error at index {idx}: {e}, path: {self.image_paths[idx]}")
            return None, None
...
class CelebA_64(DatasetFactory):
    r""" train: 162,770
         val:   19,867
         test:  19,962
         shape: 3 * width * width
    ...
    def __init__(self, path, resolution=256):
        ...
        self.train = torchvision.datasets.CelebA(root=path, split="train", target_type=[], transform=transform, download=False)
...
def get_dataset(name, **kwargs):
    if name == 'cifar10':
    ...
    elif name == 'celebahq': # celebahq
        return CelebAHQ(**kwargs)        
    elif name == 'lsun_beds': # lsun_beds
        return LSUN_B(**kwargs) 
    elif name == 'lsun_churches': # lsun_churches
        return LSUN_C(**kwargs) 
    elif name == 'ffhq': #ffhq
        return FFHQ(**kwargs)     
        ...
        # kwargs.pop('random_flip', None) # Added to remove random flip
